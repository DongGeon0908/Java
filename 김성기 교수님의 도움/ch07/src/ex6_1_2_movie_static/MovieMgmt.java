package ex6_1_2_movie_static;

/**
 * [ Movie, MovieMgmt ] : static 필드 및 메소드, 객체 배열 등의 연습
 *  
 *   o 객체 배열
 ...
 *   
 *     - 이 클래스에서는 객체 생성 및 객체 배열에 저장, 모든 객체 보기, 영화제목, 장르, 감독, 주연배우 등의 영화를 검색
 *       등의 작업을 수행
 *       
 *     - 검색은 문자열의 비교로 이루어짐
 *     
 ...
 *         
 *   o 이 프로그램에서 대부분의 메소드는 클래스 메소드로 작성
 *     - 클래스 메소드는 객체 생성하지 않고서 클래스를 수신자로 호출             
 */
...
class MovieMgmt  {
	static Movie movies[] = new Movie[200];  // 배열 변수는 반드시 < 타입명 배열변수 = new 타입명[원소개수] >로 배열 객체 생성 후 저장해야 함!!!!!

	public static void main(String[] args) {
		// Movie 객체 200개를 저장할 수 있는 배열 변수 movies 선언 및 배열 객체 저장

		setupInitialMovies();  // 주어진 영화 정보들로 여러 영화 객체들을 생성 후 movies[] 객체 배열에 저장
		...
		while(true) {			// 종료가 선택될 때까지 반복 수행
			...
			int menu  = getIntWithPrompt("선택", 1, 7);  // getIntWithPrompt()로 1에서 7 사이 메뉴 정수값 선택

			if (menu==7)  {  // 종료 선택 시
				outputResult("  !! 영화 관리를 종료합니다. Bye!!!");  // 종료 메시지 출력 후
	...
	static void setupInitialMovies() {
	...
	static Movie createAddMovie(String title, String genre, String director, String actors, int yearMade) {  // 반환타입이 Movie이므로 Movie 객체가 반환됨
		Movie m = MovieMgmt.createMovie(title, genre, director, actors, yearMade);   // 영화 객체 생성하여 객체 참조변수 m에 저장

		MovieMgmt.addMovie(m);  // m이 참조하는 객체를 movies[] 객체 배열에 저장
	...
	static void getEnrolMovie() {	
		...
		Movie m = getCreateMovie();  // 영화 정보를 입력하여 영화 객체 생성하고 생성된 객체를 반환받음
	 ...
	 * 
	 *   지금은 class Pr5_MovieArray의 메소드로 작성되지만 나중에는 SkScanner 클래스 또는 SkOutput 클래스의 메소드로
	 *   작성될 것임
	 */
	...
	// 프로그램의 부타이틀 출력(먼저 2줄을 띄후 부타이틀 출력 후 1줄을 띄움)
 ...
 *  [ 실습 과제 ]
 *  
 *    o 미완성 부분인 static void showMovieByDirector(Movie movies[])와 
 *      static void showMovieByActors(Movie movies[])을 완성하라.    
 */
