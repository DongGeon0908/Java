...
/**
 * [ PersonMgmt4 ]: Vector 클래스를 이용하여 인덱스 범위 초과 오류 방지하기
 * 
...
class Ex11_1_5_PersonMgmt4_UsingVector {  
	// Person 객체를 원소로 저장할 Vector 객체 생성하여 persons가 참조하게 함 
	static Vector<Person> persons = new Vector<Person>(); 

	// 주어진 Person 객체 p를 Vector 객체 persons에 추가함
	static void addPerson(Person p) {     
		// Vector 객체에는 원소 저장할 공간이 항상 있게 되므로 
		persons.add(p);  // p를 persons에 저장(내부적으로 저장한 원소 개수 1 증가)
	}

	// 주어진 n명의 신상정보 입력한 Person 객체를 Vector 객체 persons에 저장
	...
	// Vector 객체 persons에 저장된 모든 Person 객체를 출력
 ...
 * [ 실습과제 ]
 *   
 *   1) 다음과 같이 Integer 객체를 저장할 수 있는 Vector 객체를 생성하여 참조변수 ints가 참조하도록 하라.
 *      
 *       static Vector<Integer> ints = new Vector<Integer>(); // Integer 객체를 저장할 Vector 객체를 생성하여  ints가 참조하게 함
 *       
 *   2) 여러 정수를 입력하여 ints에 저장하도록 추가로 작성하기 위해 다음의 메소드를 작성하라.
 *      
 *        static void addInts(int n)   // n을 Vector 객체 ints의 끝에 저장하는 메소드
 *        static void inputInts(int n) // n개의 정수를 입력하여 Vector 객체 ints에 저장하는 메소드 
 *        static void inputInts()      // 정수 개수를 입력하여 그 개수의 정수를 입력해 ints에 저장하는 메소드        
 *        static void outputAllInts()  // Vector 객체 ints에 저장된 모든 정수를 출력하는 메소드 
 *        
