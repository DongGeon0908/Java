...
/**
 * [ TemperatureConversion4_GUIEvent ]: GUI로 작성된, 화씨온도를 섭씨온도로 변환하여 출력하는 프로그램
 *                                (입력 처리 함)
 *        
 *     o 온도 체계
...
class TemperatureConversion4_GUIEvent1  {
	// 필요한 컴포넌트를 배치할 윈도우 생성
	static JFrame frame = new JFrame("Temperature Conversion 4");
	...
	static void addComponents() {   
		...
		degreeConvertedTF.setFocusable(false);  // 이 텍스트필드는 입력하지 못하게 함
	}
	...
	// 화씨온도를 입력받는 텍스트필드 degreeToConvertTF에서 화씨온도가 입력되면   
	//  이를 처리하기 위해 이벤트 리스너 객체를 등록시키는 메소드
	//  degreeToConvertTF 객체에서 키보드 입력이 되면   
	//  등록된 KeyListener 객체의 이벤트 메소드가 자동적으로 호출되면서  KeyEvent 객체가 매개변수로 전달됨
	static void enrolEventListener() {
		KeyListener listener = new DegreeToConvertTF_Listener(); // 리스너 객체 생성
 ...
 *        종료 버튼을 클릭하면 윈도우가 사라지며 프로그램도 종료된다.
 ...
 *      그 이유는 입력받는 컴포넌트인 degreeToConvertTF 텍스트필드에 대한 이벤트 처리가 되었기 때문이다.
 ...
 *          DegreeToConvertTF_Listener 클래스 작성
 *          
 ...
 *      TemperatureConversion4_GUIEvent_CtoF 클래스로 작성하라. 
 *      
 *      (섭씨온도 C를 화씨온도 F로 변환하는 공식은 F = C * 9 / 5  + 32 임)
 *   
 *   2) 이 프로그램은 화씨온도를 섭씨온도로 변환하여 출력한다. 화씨온도를 캘빈 온도로 
 *      변환하여 출력하는 프로그램을 TemperatureConversion4_GUIEvent_CtoK 클래스로 작성하라.  
 *           
