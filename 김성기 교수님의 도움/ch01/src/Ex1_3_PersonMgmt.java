/**
 * [ Person, PersonMgmt ]: "홍길동은 18살에 금강산으로 갔다."라는 상황 나타내는 클래스들 작성하기
 * 
 * o 프로그래밍 스타일 1) 여러 클래스로 구성되며, 한 클래스는 여러 필드와 여러 메소드로 구성 2) 객체가 생성되어 활동함
 */
 ...
 * 
 * 이 프로그램은 지금 단계에 이해할 수 있는 프로그램이 아니라, 최종적으로 작성할 목표를 제시하는 프로그램이다.
 * 
 ...
 * [ 실습과제 ]
 *
 * o 다음의 스토리에 대해 필요한 작업을 수행하는 문장들을 Ex1_3_PersonMgmt 클래스의 main() 메소드 아래 부분에 추가로
 * 작성하고 각 객체를 출력하라. (괄호 부분은 작성할 수 없으므로 문장으로 작성하지 않는다)
 * 
 * "이름이 이몽룡인 사람이 있다. 이몽룡은 서울로 갔다. 20세가 되어 이몽룡은 (장원급제하여 암행어사가 되어 춘향을 만나러) 남원
 * 춘향집으로 갔다. (춘향이가 집에 없으니) 이몽룡은 남원 감옥으로 가 (춘향을 만났다.)
 * 
 * 이름이 성춘향인 사람이 있다. 성춘향은 16세가 되어 광한루로 놀러가 (이몽룡을 만났다. 서울로 가는 이몽룡을 눈물로 배웅하고) 춘향집으로
 * 돌아가 (지내는 중 변학도의 수청을 거부하여) 남원 감옥에 가게 되었다. 춘향은 18세에(이몽룡을 다시 만나) 서울로 가 (어떻게
 * 되었을까?)"
 *
 *
 * [ 참고사항 ]
 * 
 * 이 실습과제는 프로그램을 이해하고 작성하는 과제가 아니다. main() 메소드에 작성된 문장들을 보고 유추하여 주어진 이몽룡과 성춘향의
 * 상황 나타낼 수 있도록 추가로 작성하는 것이다.
 * 
 * 이 과제 할 수 있는 경우 상황판단 능력과 응용력이 매우 뛰어난 것이며, 할 수 없다면 하지 않아도 무방하나 한번 시도해 보기 바란다.
 */
