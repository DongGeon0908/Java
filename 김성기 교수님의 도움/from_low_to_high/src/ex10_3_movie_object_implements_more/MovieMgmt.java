package ex10_3_movie_object_implements_more;

/*
 *  [ MovieMgmt.java ]:  객체 필드 및 메소드, 객체 배열, Java.util.Vector 클래스 등의 연습
 *  
 ***************************************************************************************
 *   o 객체 배열
 ...
 *        이를 위한 기본적인 방안이 생성된 객체들을 객체 배열의 원소로 저장하는 것임
 *       
...
class MovieMgmt  {
	...
	Movie movies[] = new Movie[200];  // 배열 변수는 반드시 < 타입명 배열변수 = new 타입명[원소개수] >로 배열 객체를 생성 후 저장해야 함!!!!!

	public static void main(String[] args) {
		MovieMgmt mms = new MovieMgmt();

		mms.setupInitalMovies();  // 주어진 영화 정보들로 여러 영화 객체들을 생성 후 movies[] 객체 배열에 저장
	...
	int displayGetMenu() {
		...
		while(true) {			// 종료가 선택될 때까지 반복 수행
			...
			System.out.println("  7) 영화 삭제                          8) 종료");

			menu  = this.getIntWithPrompt("선택", 1, 8);  // getIntWithPrompt()로 1에서 7 사이의메뉴 정수값 선택

			if (menu >=1 && menu <= 8)   // 올바른 메뉴 번호가 입력되면 그 번호를 반환
				return menu;
		}
	}
	...
	Movie createAddMovie(String title, String genre, String director, String actors, int yearMade) {  // 반환타입이 Movie이므로 Movie 객체가 반환됨
		Movie m = createMovie(title, genre, director, actors, yearMade);   // 영화 객체를 생성라여 레퍼런그 변수 m에 저장

		this.addMovie(m);  // m이 레퍼런스하는 객체를 movies[] 객체 배열에 저장
...
//		return this.createMovie(title, genre, director, actors, yearMade);
	...
	// 저장된 영화 개수를 구하여 반환하는 메소드
	int getMovieCount() {
		int i;		
		for (i = 0; movies[i] != null; i++) ;  // movies 객체 배열에서 새로운 Movie 객체를 저장할, 빈 원소의 처음 위치를 찾음

		return i;  // 마지막 위치 다음의 index가 영화 개수임
	}

	// newMovie에 저장된 Movie 객체를 객체 배열 movies[]의 처음 빈곳에 저장
	void addMovie(Movie newMovie) {
		int cnt = getMovieCount();  // 저장된 영화 개수를 구함

		movies[cnt] = newMovie;  // 빈 원소의 처음 위치의 원소로 객체를 저장
	}
	...
	void showMovieByDirector() {
		...
		if (movies[0] == null) {
			...
			Movie[] moviesSearched = searchMovieByDirector(directorToSearch);
	...
	// 필드인 객체 배열 movies[]의 영화 중에서 감독이 directorToSearch를 포함하는 영화를 찾아 이들 모두를 반환
	Movie[] searchMovieByDirector(String directorToSearch) {
		int noMovies = movies.length;  // movies[] 배열 객체의 원소 개수를 구함
		...
		for (int cnt = 0; cnt < noMovies && movies[cnt] != null; cnt++)   // movies[]에 저장된 모든 영화에 대해(movies[cnt] != null이어야 실제 Movie 객체가 저장된 것임)
			if (movies[cnt].director.toLowerCase().contains(directorToSearch.toLowerCase()))   // 영화 장르가 입력된 장르를 포함하면(모두 소문자로 변경하여 포함된 것을 비교함)
				moviesSearched[cntSearched++] = movies[cnt];                             // 그 영화 객체를 moviesSearched[] 배열객체에 저장하고 cntSearched 1증가
	...
	void deleteMovie() {
		...
		int no = getIntWithPrompt("삭제할 영화의 번호", 1, getMovieCount());
		
		Movie m = deleteMovie(no - 1);
	...
	Movie deleteMovie(int n) {
		...
		Movie m = movies[n];  // 삭제될 영화를 m에 저장
		
		// n+1 위치부터의 원소들을 하나씩 앞으로 이동시킴
		int i;
		for (i = n + 1; i < cnt; i++)
			movies[i - 1] = movies[i];
		
		movies[cnt - 1] = null;  // 마지막 영화를 삭제하기 위해 null로 저장
