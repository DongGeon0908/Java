...
/*
 * [ Persons_Mgmt_UsingClass ]: 신상정보 추가, 전체 보기, 위치 이동, 나이 변경, 신상정보 삭제 등의 신상정보 관리업무 수행(틀래스 이용)
 *
 *   o 프로그래밍 스타일
 *     1) Person 클래스 작성하여 여러 작업을 수행하게 함
...
class Person {
	...
	// 이름, 나이, 현위치가 매개변수로 주어지면 이를 필드 name, age, currentLocation에 저장하는 객체 생성자
	Person(String name, int age, String location) {
		this.name = name;  	// 매개변수 name을 this 객체의 name 필드에 저장
		this.age = age;    	// 매개변수 age를 this 객체의 age 필드에 저장
		this.currentLocation = location;
	}
	// goTo(): 가다라는 행동을 구현하는 메소드
	//   주어진 장소로 가게되면 그 장소가 현위치가 되므로 
...
class Ex6_Persons_Mgmt_UsingClass {
	...
	static void addPersonInfo(String name, int age, String currentLocation) {
		Person p = new Person(name, age, currentLocation);   // 주어진 신상정보의 Person 객체 생성                   
		persons[noPersons] = p;  // persons[] 배열의 noPersons 위치에 주어진 현위치를 저장
	...
	static void deletePersonInfo(int index) {
		...
		for (int i = index; i < noPersons - 1; i++) {	
			persons[i].name = persons[i + 1].name;          // 다음 위치의 신상정보 이름을 이전 위치로 차례대로 저장하여  index 위치의 신상정보 이름 항목 삭제 
			persons[i].age = persons[i + 1].age;            // 신상정보 나이에 대해 동일하게 수행 
			persons[i].currentLocation = persons[i + 1].currentLocation;  // 신상정보 현위치에 대해 동일하게 수행 
		}
	...
	static int selectMenu() {
		...
		while (true) { // 올바른 업무에 대한 메뉴가 선택될 때까지 반복하여 업무 메뉴를 선택함 
			...
			if (menu >=1 && menu <= ENDmenu) {   // 정상적 메뉴 순번을 선택하면 메뉴 선택이 완료되므로 
				return menu;		             //     return 문을 이용하여  선택한 메뉴번호를 반환하고 메소드 종료함
			}
	...
	static void doChangeLocationMenu() { // 위치 이동 업무 
		if (noPersons == 0) {
			...
			persons[index].currentLocation = newLocation;  // no번째 신상정보의 현위치를 입력한 이동 위치로 변경시킴 
	...
	static void doDeletePersonInfoMenu() {  // 신상정보 삭제  업무
		if (noPersons == 0) {
			...
			String name = persons[index].name;  // 삭제할 사람의 이름을 name에 저장
