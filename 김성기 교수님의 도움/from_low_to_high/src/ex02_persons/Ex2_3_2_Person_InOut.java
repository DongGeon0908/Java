 ...
 *         SkScanner.getString():   엔터키가 타이핑될 때까지 타이핑된 모든 문자들을 입력
 *         SkScanner.getInt():      타이핑된 문자들 중 첫번째 정수 값을 입력, 정수가 아닌 것은 모두 무시됨
 *         SkScanner.getDouble():   타이핑된 문자들 중 첫번째 실수 값을 입력, 실수가 아닌 것은 모두 무시됨
 *         SkScanner.getChar():     문자 하나를 입력
 ...
 *         char initialOfName = SkScanner.getChar(); // 문자 입력하여 저장
...
class Ex2_3_2_Person_InOut {

	// main() 메소드는 시작 메소드로 자동적으로 호출됨
	public static void main(String[] args) {
		/*
		 *  신상정보(이름, 나이, 현위치)를 저장하기 위한 변수 선언
		 */
		...
		name = SkScanner.getString();     // SkScanner.getString()를 이용하여 문자열 입력하여 저장
		...
		/*
		 *  여러 개의 값을 한 라인에 출력, 출력할 값들을 + 로 접속하여 출력함
		 */
		System.out.println();  // 공백 라인을 출력하여 줄을 바꿈
		System.out.println("  * "  + name + "의 신상정보 : " + name + "의 나이는 " + age + "세이고 "
 ...
 *   
 *      Ex2_3_2_Person_InOut 클래스가 선언되어 있으며, 클래스 내부에 
 *      main() 메소드가 작성되어 있다.
 *      
 ...
 * [ 실습과제 ]
 *
 *   1) 키, 이름 첫 글자, 한국인 여부를 저장할 변수인 double height, char initialOfName, 
 *      boolean isKorean을 추가적으로 선언하고 키, 이름 첫 글자, 한국인 여부를 입력하여 
 *      저장한 후 이를 출력하도록 프로그램을 수정하라. 
 *      
 *      수정이 완료되면 홍길동, 이몽룡, 본인, 외국인 등에 대해 신상정보를 입력하여 
 *      결과를 확인하라.
 *  
 *   2) 본인이 좋아하는 책의 제목, 저자, 출판사, 가격, 페이지수 등의 책 정보를 
 *      저장하기 위한 변수를 각각 선언하고 각각의 변수에 값을 입력하여 저장한 후 
 *      출력하는 프로그램을 Pr2_3_Book_InOut 클래스로 작성하라.
 *      (변수 이름은 영어 단어로 작성할 것이며, 영어 단어를 모르면 인터넷 한영사전을 참조하라.
 *       또한 책 정보는 인터넷 서점에서 정확한 정보를 구하여 변수에 저장할 것임)
 *      
 *       
 * [ 참고사항 ]
 *
 *   1) 실습과제를 할 때에는 이 클래스의 프로그래밍 스타일과 동일한 스타일의 프로그램을 
 *      작성하여야 한다. 예를 들어, Pr2_3_2_Book_InOut 클래스를 작성할 때에는 main() 메소드로만 
 *      클래스 구성하고, 책 정보 항목 하나 하나를 입력하여 저장하기 위한 변수들을 선언하여야 한다.
 *      
 ...
 * [ 스스로 생각하고 고민할 과제 ]  
 *  
 *    1) Ex2_3_1_Person_Out 클래스와 같이 프로그램에서 값을 직접 변수에 저장할 경우 
 *       대상이 바뀌면 변수의 값을 바꾸어 저장해야 하므로 프로그램이 변경되어야 한다. 
 *       
 *       그런데, 이 클래스 Ex2_3_2_Person_InOut와 같이 값을 입력을 하여 변수에 저장할 경우
 *       대상이 바뀌어도 프로그램은 변경되지 않는다. 
 *       
