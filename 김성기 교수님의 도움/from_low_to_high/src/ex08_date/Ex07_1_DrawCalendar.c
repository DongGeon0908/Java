...
#include "SkScanner.h"  // 김성기 교수가 작성한 getString(), getInt(), getDouble() 등을 위한 헤더 파일 지정
...
#define DEBUG 1  // 이 값을 1로 하면  <for debug>가 출력되며, 0으로 하면 <for debug>가 출력되지 않음 
...
int main(void)  {
	...
	*/ 
	...
	year = getIntWithPrompt("  o 년 > "); // 년 입력 
	month = getIntWithPrompt("  o 월 > "); // 월 입력
