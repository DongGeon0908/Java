...
class Ex5_UnitConversion_AmazingAndHappy_UsingMethod {
	static String unitNamesFrom[] =  // 변환전 도량형 단위명을 저장하는 배열변수
		{ "", "m", "yard", "Km", "mile", "kg", "pound", "g", "don", "m*m", "pyoung" };

	static String unitNamesTo[] =   // 변환후 도량형 단위명을 저장하는 배열변수
		{ "", "yard", "m",  "mile", "Km", "pound", "kg", "don", "g", "pyoung", "m*m" };

	static double conversionFactor[] = { // 변환할 두 도량형 단위간의 변환비율
	...
	static int NOMENU = conversionFactor.length ; // 메뉴 개수를 저장하는 변수
	...
	public static void main(String[] args)  {
		...
		while(true) {
			...
			int menu = displaySelectMenu();  // 메뉴 디스플레이한  후 메뉴를 선택함
			
			if (menu == NOMENU) {   // 종료 메뉴 선택시 종료
	...
	static int displaySelectMenu() {
		...
		while(true) {
			...
			if (menu >= 1 && menu <= NOMENU)  { // 정상적  메뉴가 선택되면
