...
class Ex5_1_PlayerRecords_UsingMethod { 
	...
	static void outputTitle() {
		System.out.println("\n  ****  야구선수 정보 및 타수와 안타 기록 관리(static 변수와 메소드 사용) ****\n"); 
	}
	...
	static void getPalyernameTeam() {   
		...
		playerName = SkScanner.getString();     // static 변수는 바로 사용 가능   
		System.out.print("  o 소속 팀 이름 > ");  
		teamName = SkScanner.getString();       // static 변수는 바로 사용 가능   
    }
    ...
    static void outputPlayerRecords() { // 선수의 기록 출력\  
		System.out.println("\n\n  ** 야구선수 정보 출력 **\n");            
	...
	public static void main(String [] args) {
		outputTitle();         // 프로그램 타이틀 출력
        getPalyernameTeam();   // 선수의 이름과 팀명 입력
        getPalyrRecords(5);    // 5 경기에서의 선수 기록 입력
        outputPlayerRecords(); // 선수의 기록 출력
	}
}
...
* [ 프로그램의 이해 및 실행 ]
*
*   1) 이 프로그램은 메소드를 사용하여  간결하게 프로그램을 작성하였다.
*
*   2) 각 메소드의 기능을 이해하고 그 기능을 위해 작성된 구현부분을 파악하라.
*      특히, 메소드 외부에 저장된 static 변수를 static 메소드 내에서 자유롭게 사용할 수 있음을 반드시 확인하라.
*   
